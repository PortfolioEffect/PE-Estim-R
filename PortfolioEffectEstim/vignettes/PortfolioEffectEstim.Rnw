...
{\huge User Manual \\}
\vspace{0.3cm}
{\Huge\bfseries PortfolioEffectEstim \\ R Package \\}
\vspace{0.4cm}
...
PortfolioEffectEstim package for R relies on the rJava package, which assumes that Java runtime is installed and configured on your system.
...
If you are manually installing PortfolioEffectEstim package (you don't want to use CRAN
...
You are now ready to install the PortfolioEffectEstim package directly from 
...
Run the following commands to set your account API credentials for the PortfolioEffectEstim R Package installed. 
...
this procedure if you change your account password or install PortfolioEffectEstim package
on another computer.
\begin{lstlisting}
